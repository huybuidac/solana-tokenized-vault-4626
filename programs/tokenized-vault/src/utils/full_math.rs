...
use crate::utils::big_num::{U128, U256, U512};
...
pub trait MulDiv<RHS = Self> {
    ...
    /// Calculates `val * num / denom` with a rounding direction.
    fn mul_div(self, num: RHS, denom: RHS, rounding: Rounding) -> Option<Self::Output>;
...
impl MulDiv for u64 {
    ...
    fn mul_div(self, num: Self, denom: Self, rounding: Rounding) -> Option<Self::Output> {
        match rounding {
            Rounding::Floor => self.mul_div_floor(num, denom),
            Rounding::Ceiling => self.mul_div_ceil(num, denom),
        }
    }

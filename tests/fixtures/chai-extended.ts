...
chai.use(function (chai, utils) {
  ...
  Assertion.addMethod("closeToBN", function (value, delta) {
    ...
    const result =
      _obj < _value ? _value - _obj <= _delta : _obj - _value <= _delta;
    new Assertion(
      result,
  ...
  Assertion.addMethod(
    "log",
    function (provider: LiteSVMProvider, enabled: boolean = true) {
      ...
      const derivedPromise = _promise.then(
        (result: any) => {
          if (enabled) {
            ...
            if (hash) {
              const tx = provider.client.getTransaction(
      ...
      const derivedPromise = _promise.then(
        (result: any) => {
          for (const changes of befores) {
            const af = provider.client.getBalance(
              new PublicKey(changes.account)
            );
    ...
    function (
      ...
      const befores = changes.map((change) => {
        ...
        const account = wallet
          ? wrapper.getAtaOf(new PublicKey(wallet))
